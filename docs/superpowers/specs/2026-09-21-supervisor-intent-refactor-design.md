...
| 风险 | 说明 | 缓解 |
|---|---|---|
| **`needs_lightweight_recall` 兜底消失（实现后复核：本表最大的一条；有最小缓解，但仍无量化手段）** | 改造前 `route_from_supervisor` 在**路由阶段**把 `detect_memory_intent(...).get("needs_lightweight_recall")` 当 OR 条件用，所以「那个作业后来怎么样了」这类没有显式回忆词的消息**一定**会进 memory 节点。改造后这个信号还在算（`memory_agent.py:240`），但它只能在 memory 节点**内部**起作用——分类器判 `none` 时该节点根本不跑，那三个 OR 条件一个都到不了。所以这是一次真实的召回面收窄，不是等价改写 | 原计划写的是「标注集中必须包含这类样本」，但标注集已随 §7 一并推迟（没有真实数据），**量化缓解不存在**。最小可用替代已于 2026-09-21 落地：给分类器提示词的 `recall` 定义补了一条「不需要显式回忆词的指代性追问」，并附上「那个作业后来怎么样了」作为少样本例子。注意这**只是把守卫补回去，不是把风险量化掉**——模型是否照做仍然没有测量手段，有数据前这仍是一处未测量的收窄 |
| **每条消息多一次 LLM 调用** | 从「偶发」变成「每条」 | 快速通道兜住高频短消息；延迟与成本用标注集量化。**2026-09-21 补**：即便还没量化，尾延迟也不再无界了——全链路补了分层超时 + 90s 轮级 deadline（见下条）。但注意这只约束**最坏情况**，不改**期望延迟**：每条消息多一次分类调用的常规成本仍在，那是标注集要测的东西 |
...
| **超时值是估的，还没标定** | 2026-09-21 补的分层超时（分类器 5s / embedding 8s / 图内辅助 LLM 12s / 回复 45s / 轮级 90s）是按调用类型估的起点，不是从实测分布推出来的。定得太松等于没兜底，太紧会把正常慢路径砍掉——`CHAT_TURN_DEADLINE` 尤其，它一响用户这轮什么都收不到 | **保证的是「有界」，不是「刚好」**。**2026-09-21 实测（端到端，不是 p99）**：真实模型 + 真实 LanceDB/FTS，一个需要翻记忆的提问，整轮 5.7–6.4s（三次）。对 90s 的 deadline 是 ~14 倍余量，说明 deadline 不会误伤正常慢路径——但也说明它只在**病态**情况下才会响，这正是它该有的位置。**分层值仍未标定**：要按 span 分布定这些数，得读 LangSmith，而该账号的**月度 trace 额度已经用完**（`Monthly unique traces usage limit exceeded`），在额度恢复前这条做不了。届时的办法不变：跑若干轮真实对话（含一次需要翻记忆的提问、一次带图提问），读各 span 时长，把每层设到观测 p99 的 ~3 倍 |
| **memory 节点降级后用户无感** | 2026-09-21 给 memory 加了分阶段兜底：检索失败时静默少一块记忆，照常回答（`reply_provenance` 里看不到，只进 trace 和日志） | 这是**有意的**：降级信息对用户没用，只对排查有用。代价是降级率涨了不会有人从界面上发现，唯一会自己冒出来的信号是 trace/日志——所以 `error` 字段记的是异常类名 + 原文而不是原因码，超时和鉴权失败要能一眼分开 |
## 7. 验证方案
