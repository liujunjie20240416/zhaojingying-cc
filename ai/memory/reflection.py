...
事实分类（必须四选一，不允许其他分类）：
- identity: 用户的恒定身份（姓名/生日/出生地/家庭成员等不会随时间改变的信息）
- preference: 用户的偏好习惯（喜欢/讨厌/习惯/饮食/爱好/审美等），以及当前状态身份（现在/目前的学校/职业/城市/学历/证书/经济状况等），可能随时间改变
- experience: 用户经历过的具体事件（过去发生的事/共同回忆/重大事件），以及已结束的过去状态（曾就读/曾任职的学校/职业/城市等），不可改变
...
14. 状态身份按时态分流：当前状态（用户说"现在/目前"，如"用户目前在B大学读研"）归入 preference，同维度变化时用 replaces 把旧状态转为历史、保留轨迹；已结束的过去状态（用户说"以前/曾经"，如"用户2021至2024年在A大学就读"）归入 experience，追加即可、不可变，不要用 replaces
15. 情绪化自嘲/抱怨/自我贬低（如"一穷二白""啥证书没有"）是临时情绪表达，不要提炼为长期事实；只提取其中具体的客观信息"""
